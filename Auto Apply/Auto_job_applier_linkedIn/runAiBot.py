...
def apply_filters() -> None:
    ...
    try:
        ...
        def click_filter_option(option_text, section_heading=None, timeout=10):
            """Directly click a filter option by label text within the All Filters modal.
            If section_heading is given, restricts to that section only.
            Otherwise tries globally within the modal."""
            print_lg(f"=== [FILTER] Trying to click '{option_text}' (section='{section_heading}') ===")
            try:
                modal = WebDriverWait(driver, timeout).until(
                    EC.presence_of_element_located((By.XPATH, '//*[contains(@class,"jobs-easy-apply-modal") or contains(@class,"search-reusables__filter-results-container") or contains(@id,"search-reusables-filters") or @aria-label="All filters" or @data-test-modal]'))
                )
            except Exception:
                print_lg(f"[FILTER] Could not find modal container, using driver root")
                modal = driver

            if section_heading:
                # find the section by heading text
                section_xp = (
                    f'//*[self::h3 or self::h2 or self::h4 or self::legend or self::span]'
                    f'[normalize-space(text())="{section_heading}" or normalize-space(.)="{section_heading}"]'
                )
                try:
                    heading_el = WebDriverWait(driver, 5).until(EC.presence_of_element_located((By.XPATH, section_xp)))
                    # get the parent fieldset/li/div of that heading
                    section_el = driver.execute_script(
                        """var el = arguments[0];
                        while (el && el.tagName !== 'FIELDSET' && el.tagName !== 'LI' && !el.classList.contains('artdeco-accordion')) {
                            el = el.parentElement;
                        }
                        return el;""", heading_el)
                    print_lg(f"[FILTER] Found section '{section_heading}' via heading, tag={section_el.tag_name if section_el else 'None'}")
                    if section_el:
                        modal = section_el
                except Exception as se:
                    print_lg(f"[FILTER] Could not find section heading '{section_heading}': {se}")

            # Now inside modal/section, find label with matching text, then click it
            try:
                # Try clicking a label directly - use contains() since labels may have visually-hidden nested text
                label_xp = f'.//label[contains(normalize-space(.), "{option_text}")]'
                print_lg(f"[FILTER] Searching for label with XPath: {label_xp}")
                label_els = modal.find_elements(By.XPATH, label_xp)
                print_lg(f"[FILTER] Found {len(label_els)} label(s) matching '{option_text}'")
                if label_els:
                    lbl = label_els[0]
                    scroll_to_view(driver, lbl)
                    try:
                        lbl.click()
                        print_lg(f"[FILTER] Label click successful.")
                    except Exception:
                        driver.execute_script("arguments[0].click();", lbl)
                        print_lg(f"[FILTER] Label JS click fallback used.")
                    buffer(click_gap)
                    print_lg(f"=== [FILTER] Successfully clicked '{option_text}' ===")
                    return True
                else:
                    # Try span text match using contains()
                    span_xp = f'.//span[contains(normalize-space(.), "{option_text}") and not(ancestor::span)]'
                    span_els = modal.find_elements(By.XPATH, span_xp)
                    print_lg(f"[FILTER] Trying span XPath with contains(), found {len(span_els)} span(s)")
                    if span_els:
                        sp = span_els[0]
                        scroll_to_view(driver, sp)
                        try:
                            sp.click()
                        except Exception:
                            driver.execute_script("arguments[0].click();", sp)
                        buffer(click_gap)
                        print_lg(f"=== [FILTER] Successfully clicked span '{option_text}' ===")
                        return True
                    # Final fallback: find radio input whose associated label contains the text
                    try:
                        radio_xp = f'.//input[@type="radio" or @type="checkbox"]'
                        radios = modal.find_elements(By.XPATH, radio_xp)
                        print_lg(f"[FILTER] Trying radio fallback, found {len(radios)} radio/checkboxes")
                        for radio in radios:
                            rid = radio.get_attribute("id")
                            if rid:
                                try:
                                    assoc_lbl = modal.find_element(By.XPATH, f'.//label[@for="{rid}"]')
                                    if option_text.lower() in assoc_lbl.text.lower():
                                        print_lg(f"[FILTER] Found radio with label containing '{option_text}': '{assoc_lbl.text.strip()}'")
                                        scroll_to_view(driver, assoc_lbl)
                                        try:
                                            assoc_lbl.click()
                                        except Exception:
                                            driver.execute_script("arguments[0].click();", assoc_lbl)
                                        buffer(click_gap)
                                        print_lg(f"=== [FILTER] Radio fallback click successful for '{option_text}' ===")
                                        return True
                                except Exception:
                                    pass
                    except Exception as re:
                        print_lg(f"[FILTER] Radio fallback failed: {re}")
                    print_lg(f"=== [FILTER] No matching element found for '{option_text}' ===")
                    return False
            except Exception as e:
                print_lg(f"=== [FILTER] Exception clicking '{option_text}': {e} ===")
                return False

        # Date posted
        if date_posted:
            print_lg(f"[FILTER] Setting Date posted = '{date_posted}'")
            if not click_filter_option(date_posted, section_heading="Date posted", timeout=10):
                print_lg("[FILTER] Section-scoped click failed, trying global fallback for date_posted")
                click_filter_option(date_posted, timeout=10)
        buffer(recommended_wait)

        # Experience level — scoped to its section
        for lvl in experience_level:
            print_lg(f"[FILTER] Setting Experience level = '{lvl}'")
            if not click_filter_option(lvl, section_heading="Experience level"):
                print_lg(f"[FILTER] Section-scoped click failed for Experience level '{lvl}', falling back")
                wait_span_click(driver, lvl, 8)
        multi_sel_noWait(driver, companies, actions)
        if experience_level or companies: buffer(recommended_wait)

        # Job type — scoped to prevent clicking wrong 'Internship' in Experience section
        for jt in job_type:
            print_lg(f"[FILTER] Setting Job type = '{jt}'")
            if not click_filter_option(jt, section_heading="Job type"):
                print_lg(f"[FILTER] Section-scoped click failed for Job type '{jt}', falling back")
                wait_span_click(driver, jt, 8)

        # On-site/Remote
        for os_opt in on_site:
            print_lg(f"[FILTER] Setting On-site/Remote = '{os_opt}'")
            if not click_filter_option(os_opt, section_heading="On-site/Remote"):
                if not click_filter_option(os_opt, section_heading="Remote"):
                    if not click_filter_option(os_opt, section_heading="Workplace type"):
                        # Global fallback - no section scoping
                        print_lg(f"[FILTER] All section-scoped clicks failed for On-site '{os_opt}', trying global")
                        if not click_filter_option(os_opt):
                            wait_span_click(driver, os_opt, 8)
        if job_type or on_site: buffer(recommended_wait)

        try:
            if easy_apply_only: boolean_button_click(driver, actions, "Easy Apply")
        except Exception as ef: print_lg(f"[FILTER] easy_apply toggle failed: {ef}")

        try:
            multi_sel_noWait(driver, location)
            multi_sel_noWait(driver, industry)
            if location or industry: buffer(recommended_wait)
        except Exception as ef: print_lg(f"[FILTER] location/industry filter failed (non-fatal): {ef}")

        try:
            multi_sel_noWait(driver, job_function)
            multi_sel_noWait(driver, job_titles)
            if job_function or job_titles: buffer(recommended_wait)
        except Exception as ef: print_lg(f"[FILTER] job_function/titles filter failed (non-fatal): {ef}")
        ...
        except Exception as ef: print_lg(f"[FILTER] boolean filter failed (non-fatal): {ef}")

        try:
            wait_span_click(driver, salary)
            buffer(recommended_wait)
        except Exception as ef: print_lg(f"[FILTER] salary filter failed (non-fatal): {ef}")

        try:
            multi_sel_noWait(driver, benefits)
            multi_sel_noWait(driver, commitments)
            if benefits or commitments: buffer(recommended_wait)
        except Exception as ef: print_lg(f"[FILTER] benefits/commitments filter failed (non-fatal): {ef}")

        # ALWAYS click Show Results — this must happen even if some filters above failed
        ...
        print_lg("[FILTER] Clicked 'Show results' button successfully.")
...
def get_job_main_details(job: WebElement, blacklisted_companies: set, rejected_jobs: set) -> tuple[str, str, str, str, str, bool]:
    ...
    * skip: A boolean flag to skip this job
    '''
    # Get job_id FIRST from the li element attribute — safe before any scrolling
    job_id = job.get_dom_attribute('data-occludable-job-id')

    # Scroll to the job card (this may trigger LinkedIn's virtual list re-render)
    job_details_button = job.find_element(By.TAG_NAME, 'a')
    scroll_to_view(driver, job_details_button, True)

    # After scrolling, LinkedIn's virtual list may re-render, making `job` stale.
    # ALWAYS re-fetch by job_id — if this fails, let the StaleElementReferenceException
    # propagate so the calling loop can handle it (not silently use a dead reference).
    job = driver.find_element(By.XPATH, f"//li[@data-occludable-job-id='{job_id}']")
    job_details_button = job.find_element(By.TAG_NAME, 'a')
    ...
    # Title-based filter: skip if job title contains non-intern/senior words
    if not skip:
        title_lower = title.lower()
        for word in title_bad_words:
            if word.lower() in title_lower:
                print_lg(f'Skipping "{title} | {company}" (Title contains blocked word "{word}"). Job ID: {job_id}!')
                skip = True
                break
...
def get_job_description(
    ...
    try:
        ...
        found_masters = 0
        # Use JS innerText to exclude hidden <code> elements (LinkedIn embeds API JSON in them)
        desc_el = find_by_class(driver, "jobs-box__html-content")
        jobDescription = driver.execute_script("return arguments[0].innerText;", desc_el) or ""
        ...
        try:
            while current_count < switch_number:
                ...
                try:
                    ...
                    print_lg("[JOB LOOP] StaleElement waiting for job listings, retrying...")
                    buffer(2)
                    wait.until(EC.presence_of_all_elements_located((By.XPATH, "//li[@data-occludable-job-id]")))

                pagination_element, current_page = get_page_info()

                # Collect ONLY job ID strings — not WebElement refs.
                # LinkedIn's virtual list destroys/recreates DOM nodes on scroll,
                # making any stored WebElement ref immediately stale.
                # Strings never go stale: we re-fetch each element fresh by ID in the loop.
                buffer(3)
                raw_els = driver.find_elements(By.XPATH, "//li[@data-occludable-job-id]")
                job_id_list = []
                for el in raw_els:
                    try:
                        jid = el.get_attribute("data-occludable-job-id")
                        if jid:
                            job_id_list.append(jid)
                    except StaleElementReferenceException:
                        pass  # Already gone, skip it
                stale_count = 0

                for job_id_iter in job_id_list:
                    if keep_screen_awake: pyautogui.press('shiftright')
                    if current_count >= switch_number: break
                    print_lg("\n-@-\n")

                    # Re-fetch a live element reference right before using it
                    try:
                        job = driver.find_element(By.XPATH, f"//li[@data-occludable-job-id='{job_id_iter}']")
                    except Exception:
                        print_lg(f"[JOB LOOP] Job {job_id_iter} no longer in DOM (scrolled off), skipping...")
                        continue

                    try:
                        job_id,title,company,work_location,work_style,skip = get_job_main_details(job, blacklisted_companies, rejected_jobs)
                    except StaleElementReferenceException:
                        # Virtual list re-rendered mid-scroll — retry once with fresh fetch
                        try:
                            job = driver.find_element(By.XPATH, f"//li[@data-occludable-job-id='{job_id_iter}']")
                            job_id,title,company,work_location,work_style,skip = get_job_main_details(job, blacklisted_companies, rejected_jobs)
                        except Exception as retry_e:
                            print_lg(f"[JOB LOOP] Retry also failed for job {job_id_iter}: {retry_e}, skipping")
                            stale_count += 1
                            continue
                    except Exception as job_detail_e:
                        print_lg(f"[JOB LOOP] Error reading job details: {job_detail_e}, skipping")
                        continue
                    ...
                    if try_xp(driver, ".//button[contains(@class,'jobs-apply-button') and contains(@class, 'artdeco-button--3') and contains(@aria-label, 'Easy')]"):
                        try: 
                            try:
                                ...
                                # Check if this is a multi-page form (has Next/Review) or single-page (goes straight to Submit)
                                def _has_next_or_review():
                                    try:
                                        modal.find_element(By.XPATH, './/span[normalize-space(.)="Review"]')
                                        return True
                                    except NoSuchElementException: pass
                                    try:
                                        modal.find_element(By.XPATH, './/button[.//span[normalize-space(.)="Next"]]')
                                        return True
                                    except NoSuchElementException: pass
                                    return False

                                # First page: answer any questions and try to advance
                                questions_list = answer_questions(modal, questions_list, work_location, job_description=description)
                                if useNewResume and not uploaded: uploaded, resume = upload_resume(modal, default_resume_path)

                                if _has_next_or_review():
                                    # Try to click Next on the first page
                                    wait_span_click(modal, "Next", 1)

                                    while _has_next_or_review():
                                        next_counter += 1
                                        if next_counter >= 15: 
                ...
                if stale_count >= len(job_id_list) and len(job_id_list) > 0:
                    # All jobs were stale — page probably re-rendered. Buffer and retry the while loop.
                    print_lg(f"[JOB LOOP] All {stale_count} jobs were stale. Waiting for page to settle...")
                    buffer(4)
                    continue
                if pagination_element == None:
                    ...
                    break
                try:
                    # Re-fetch pagination to avoid stale reference
                    fresh_pagination = try_find_by_classes(driver, ["jobs-search-pagination__pages", "artdeco-pagination", "artdeco-pagination__pages"])
                    fresh_pagination.find_element(By.XPATH, f"//button[@aria-label='Page {current_page+1}']").click()
            ...
            # Stale element hit outside the per-job handler — not a dead session, just skip this search term
            print_lg(f"[JOB LOOP] StaleElementReferenceException outside job loop, skipping search term: {e}")
        except (NoSuchWindowException, WebDriverException) as e:
            # Check if it's really a dead session or just wrapped stale element
            err_msg = str(e).lower()
            if 'stale element' in err_msg:
                print_lg(f"[JOB LOOP] WebDriverException wrapping stale element, skipping search term")
            else:
                print_lg("Browser window closed or session is invalid. Ending application process.", e)
                raise e # Re-raise only for true session death
        except Exception as e:
