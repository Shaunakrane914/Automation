...
def apply_filters() -> None:
    ...
    try:
        ...
        def click_in_section(section_heading, option_text, timeout=8):
            """Click an option within a named filter section to avoid cross-section false matches."""
            print_lg(f"=== [DEBUG] Attempting to click '{option_text}' in section '{section_heading}' ===")
            tags = ["h2", "h3", "h4", "legend", "span"]
            xp_parts = []
            for tag in tags:
                xp_parts.append(f'.//{tag}[normalize-space(.)="{section_heading}"]/ancestor::fieldset')
                xp_parts.append(f'.//{tag}[normalize-space(.)="{section_heading}"]/ancestor::li')
                xp_parts.append(f'.//{tag}[normalize-space(.)="{section_heading}"]/ancestor::div[contains(@class,"filter")]')
            xp = " | ".join(xp_parts)
            print_lg(f"[DEBUG] Using section XPath: {xp}")
            try:
                print_lg(f"[DEBUG] Waiting up to {timeout}s for section presence...")
                section = WebDriverWait(driver, timeout).until(EC.presence_of_element_located((By.XPATH, xp)))
                print_lg(f"[DEBUG] Found section element! Tag: {section.tag_name}, Class: {section.get_attribute('class')}")
                btn_xpath = f'.//label[normalize-space(.)="{option_text}"] | .//span[normalize-space(.)="{option_text}"] | .//button[normalize-space(.)="{option_text}"]'
                print_lg(f"[DEBUG] Looking for button with XPath: {btn_xpath}")
                btn = section.find_element(By.XPATH, btn_xpath)
                print_lg(f"[DEBUG] Found button element! Tag: {btn.tag_name}")
                scroll_to_view(driver, btn)
                print_lg(f"[DEBUG] Scrolled button into view. Attempting standard click...")
                try:
                    btn.click()
                    print_lg(f"[DEBUG] Standard click successful.")
                except Exception as click_err:
                    print_lg(f"[DEBUG] Standard click failed: {click_err}. Attempting JS click fallback...")
                    # fallback to javascript click if standard click is intercepted/hidden
                    driver.execute_script("arguments[0].click();", btn)
                    print_lg(f"[DEBUG] JS click fallback executed.")
                buffer(click_gap)
                print_lg(f"=== [DEBUG] Successfully completed click for '{option_text}' in '{section_heading}' ===")
                return True
            except Exception as e:
                print_lg(f"=== [DEBUG] Failed to click {option_text} in {section_heading}: {e} ===")
                return False

        # Date posted — scoped to avoid clicking wrong span
        if date_posted:
            if not click_in_section("Date posted", date_posted, 10):
                wait_span_click(driver, date_posted, 10)  # fallback
        buffer(recommended_wait)

        # Experience level — scoped to its section
        for lvl in experience_level:
            if not click_in_section("Experience level", lvl):
                wait_span_click(driver, lvl, 8)  # fallback
        multi_sel_noWait(driver, companies, actions)
        if experience_level or companies: buffer(recommended_wait)

        # Job type — scoped to its section (prevents clicking Experience level's 'Internship')
        for jt in job_type:
            if not click_in_section("Job type", jt):
                wait_span_click(driver, jt, 8)  # fallback

        # On-site — scoped to its section
        for os_opt in on_site:
            if not click_in_section("On-site/Remote", os_opt):
                if not click_in_section("Remote", os_opt):
                    wait_span_click(driver, os_opt, 8)  # fallback
